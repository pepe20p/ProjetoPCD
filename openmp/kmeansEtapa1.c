...
static void update_step_1d(const double *X, double *C, const int *assign, int N, int K){
    ...
    double *sum_thread = (double*)calloc((size_t)K*threads, sizeof(double));
    int *cnt_thread = (int*)calloc((size_t)K*threads, sizeof(int));
    #pragma omp parallel
    {
        int tid = omp_get_thread_num();
        double *soma_interna = &sum_thread[tid*K];
        int *cnt_interno = &cnt_thread[tid*K];
    ...
    for(int c=0; c<K; c++){
        for(int t=0; t<threads; t++){
            sum[c] += sum_thread[t * K + c];
            cnt[c] += cnt_thread[t * K + c];
        }
    }
    free(sum_thread);
    free(cnt_thread);
...
int main(int argc, char **argv){
    ...
    printf("K-means 1D (naive) - ETAPA 1\n");
